...
import { Ban, RefreshCw, Shield, ShieldAlert, AlertTriangle } from "lucide-react";
...
export default function DeviceTrustPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/security/device-trust", { cache: "no-store" });
        ...
        <section className="relative ml-72 p-6">
          <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-black text-amber-400">Device Trust</h1>
                <p className="mt-1 text-sm text-zinc-300">Device trust scoring, trusted device management and fingerprint tracking.</p>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
              <MetricCard label="Total devices" value={kpis.totalDevices || 0} detail="registered" icon={Shield} />
              <MetricCard label="Trusted" value={kpis.trustedDevices || 0} detail="score >= 70" icon={ShieldAlert} tone="green" />
              <MetricCard label="Untrusted" value={kpis.untrustedDevices || 0} detail="score < 30" icon={Ban} tone="red" />
              <MetricCard label="Pending" value={kpis.pendingReview || 0} detail="30-70 range" icon={AlertTriangle} />
            </div>

            <div className="grid grid-cols-1 gap-6 xl:grid-cols-[2fr_1fr]">
              <Panel title="Trust score trend" subtitle="7d">
                <div className="h-72">
                  <ResponsiveContainer>
                    <AreaChart data={data.trustTrend || []}>
                      <XAxis dataKey="day" stroke="#a1a1aa" fontSize={11} />
                      <YAxis stroke="#a1a1aa" fontSize={11} />
                      <Tooltip contentStyle={{ background: "#090d13", border: "1px solid #78350f" }} />
                      <Area type="monotone" dataKey="score" stroke="#22c55e" fill="#22c55e33" strokeWidth={3} />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </Panel>
              <Panel title="Device trust" subtitle="live">
                <div className="h-72">
                  <ResponsiveContainer>
                    <PieChart>
                      <Pie data={data.distribution || []} dataKey="value" innerRadius={72} outerRadius={105} paddingAngle={4}>
                        {(data.distribution || []).map((entry: AnyRecord) => (
            ...
            <Panel title="Device trust list" subtitle="all devices">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead className="border-b border-zinc-800 text-zinc-400">
                    <tr>
                      <th className="pb-3 font-black uppercase">User</th>
                      <th className="pb-3 font-black uppercase">Device ID</th>
                      <th className="pb-3 font-black uppercase">Trust</th>
                      <th className="pb-3 font-black uppercase">Country</th>
                      <th className="pb-3 font-black uppercase">Last Seen</th>
                    </tr>
                  </thead>
                  <tbody className="text-zinc-100">
                    {(data.devices || []).map((device: AnyRecord) => (
                      <tr key={device.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                        <td className="py-3">{device.email}</td>
                        <td className="py-3 font-mono text-xs">{device.deviceId?.slice(0, 16)}...</td>
                        <td className="py-3">
                          <span className={`rounded-full border px-2 py-0.5 text-xs font-bold ${
                            ...
                            device.trustScore < 30 ? "border-red-500/40 bg-red-500/15 text-red-300" :
                            "border-yellow-500/40 bg-yellow-500/15 text-yellow-300"
                          }`}>
                            {device.trustScore}
                          </span>
                        </td>
                        <td className="py-3">{device.country || "Unknown"}</td>
                        <td className="py-3">{formatTime(device.lastSeenAt)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
