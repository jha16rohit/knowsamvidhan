            ...
            <div ref={filterDropdownRef} className="relative shrink-0">
              ...
              {isFilterDropdownOpen && (
                <div className="absolute z-50 mt-2 w-full min-w-55 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                  ...
                  >
                    All Parts
                    {selectedPartFilter === 'all' && <Check className="w-4 h-4 text-[#f59e0b]" />}
                  </div>

                  {availableParts.map((part) => (
                    <div
                      key={part.id}
                      onClick={() => { setSelectedPartFilter(part.id); setIsFilterDropdownOpen(false); }}
                      className={`px-4 py-3 text-sm cursor-pointer transition flex items-center justify-between ${
                    ...
                    >
                      {part.label}
                      {selectedPartFilter === part.id && <Check className="w-4 h-4 text-[#f59e0b]" />}
                    </div>
                  ))}
                </div>
              )}
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {articles.map((article) => (
                        <tr key={article.id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-6 py-5 text-sm font-medium text-gray-900">{article.articleNumber}</td>
              ...
              <form id="article-form" onSubmit={handleSave} className="space-y-6">
                    ...
                    {isPartDropdownOpen && (
                      <div className="absolute z-50 mt-2 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto">
                        {availableParts.map((partOption) => (
                          ...
                          >
                            {partOption.label}
                            {formData.part === partOption.id && <Check className="w-4 h-4 text-[#f59e0b]" />}
                          </div>
                        ))}
                      </div>
                    )}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="text" required placeholder="e.g. Article 21"
                      value={formData.id}
                  ...
                  <div>
                    ...
                    <input
                      type="text" required placeholder="e.g. Right to life and personal liberty"
                ...
                <div>
                  ...
                  <textarea rows={2} value={formData.shortSummary}
                ...
                <div>
                  ...
                  <textarea rows={3} value={formData.officialText}
                ...
                <div>
                  ...
                  <textarea rows={3} value={formData.simpleExplanation}
                ...
                <div>
                  ...
                  <textarea rows={2} value={formData.example}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pb-2">
                  <div>
                    ...
                    <input
                      type="text" placeholder="Comma-separated, e.g. Liberty, Privacy"
                      value={formData.tags}
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-7 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-[15px] text-gray-500 leading-relaxed mb-8">
              This will permanently remove the article from the platform. Any clauses attached to it will become orphaned. This action cannot be undone.
            </p>
